...
- **Budget Firecrawl** : une ligne, toujours présente, jamais plus longue —
  ...
  plus le solde restant (`scripts/fc.sh solde`) et l'économie du cache
  (`SELECT sum(credits_economises) FROM v_firecrawl_cache;`). Format : « Firecrawl :
  N crédits (kiosque N /search, instructeur N /scrape…), M refus plafond, C appels
  servis par le cache, solde S ». C'est la seule dépense réelle du système : elle se
  lit tous les matins, pas une fois le stock vidé.
